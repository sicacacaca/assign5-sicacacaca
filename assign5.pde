...
void draw() {
    ...
    addTime(CLOCK_BONUS_SECONDS);
		...
		for(int i = 0; i < soldierX.length; i++){
			...
			// Requirement #3: Use boolean isHit(...) to detect collision
			if(isHit (  playerX, playerY, playerX+SOIL_SIZE , playerY+SOIL_SIZE  ,soldierX[i],soldierY[i], soldierX[i] +  SOIL_SIZE, soldierY[i] + SOIL_SIZE)==true) { // r1 bottom edge past r2 top
